...
int CountingMethods::f[MAX_INPUT_N];

std::vector<int> CountingMethods::subgraph;

int CountingMethods::K; // Subgraph size
int CountingMethods::V; // Initial node
std::chrono::time_point<std::chrono::steady_clock> CountingMethods::startAlgo;

// Used in classical graph
vector< vector<int> > CountingMethods::g;

 //Used in Hypergraph
Hypergraph CountingMethods::h;
map< int, long long> CountingMethods::counterHyper;
std::unordered_set< vector<int>, HashFunction> CountingMethods::visited;
...
void CountingMethods::enumerateSubgraphs(vector<int> extension) {
...
void CountingMethods::setupAndRun(const vector< vector<int> >& inputGraph, int k) {
  K = k;
  g = inputGraph;
...
std::map< int, long long> CountingMethods::bruteForce3(Hypergraph& inputGraph) {
  clearDataStruct();
...
std::map< int, long long> CountingMethods::bruteForce4(Hypergraph& inputGraph) {
  clearDataStruct();
  const int n = inputGraph.getNodeCount();
  vector<int> nodes;
  for (int i = 0; i < n; i++) {
...
map<string, long long> CountingMethods::FaSE(const vector<pair<int, int> > edges, int k) {
  // Set up FaSE usage
...
Hypergraph CountingMethods::binaryToHyper(string str, int k) {
  vector< vector<int> > adj;
  for (int i = 0; i < k; i++) {
    for (int j = 0; j < i; j++) {
      if (str[i * k + j] == '1') {
...
std::map< int, long long> CountingMethods::k3(Hypergraph& inputGraph) {
  clearDataStruct();
...
void CountingMethods::k3IntermediateForm(Hypergraph& inputGraph) {
  for (auto& edge : inputGraph.getIncidenceMatrix()) { // assuming no duplicate edges ...
    if (edge.size() != 3) continue;
    Hypergraph motif = inputGraph.induceSubgraph(edge);
...
std::map< int, long long> CountingMethods::k3Triangle(Hypergraph& inputGraph) {
  clearDataStruct();
...
std::map< int, long long> CountingMethods::k3Fase(Hypergraph& inputGraph) {
  clearDataStruct();
  k3IntermediateForm(inputGraph);
...
std::map< int, long long> CountingMethods::k3ESU(Hypergraph& inputGraph) {
  clearDataStruct();
  k3IntermediateForm(inputGraph);
  Search = GRAPH;
...
std::map< int, long long> CountingMethods::k4(Hypergraph& inputGraph) {
  clearDataStruct();
  for (auto& edge : inputGraph.getIncidenceMatrix()) { // assuming no duplicate edges ...
...
void CountingMethods::k4IntermediateForm(Hypergraph& inputGraph) {
  for (auto& edge : inputGraph.getIncidenceMatrix()) { // assuming no duplicate edges ...
    if (edge.size() != 4) continue;
    Hypergraph motif = inputGraph.induceSubgraph(edge);
    if (motif.is_two_connected()) {
...
// FaSE
std::map< int, long long> CountingMethods::k4Fase(Hypergraph& inputGraph) {
  clearDataStruct();
...
std::map< int, long long> CountingMethods::k4ESU(Hypergraph& inputGraph) {
  clearDataStruct();
  k4IntermediateForm(inputGraph);
  Search = GRAPH;
  h = inputGraph.filterEdge(2);
...
void CountingMethods::printResults(std::chrono::time_point<std::chrono::steady_clock> startTime, std::chrono::time_point<std::chrono::steady_clock> endTime, map< int, long long> subgraph_count, int k, bool detailedOutput, ostream& out) {
...
void CountingMethods::clearDataStruct() {
  subgraph.clear();
  counterHyper.clear();
  visited.clear();
}
...
void CountingMethods::networkCensus(Hypergraph& h, int motifSize, bool detailedOutput, int algorithm, ostream& out) {
  ...
  if (motifSize == 3) { // Execute our fastest method K = 3, TRIANGLE
    k = 3;
    switch (algorithm) {
      case 1: CountingMethods::k3(h); break;
      case 2: CountingMethods::k3ESU(h); break;
      case 3: CountingMethods::k3Triangle(h); break;
      case 4: CountingMethods::k3Fase(h); break;
      default: CountingMethods::k3Triangle(h); break;
    }
  } else { // Execute our fastest method K=4, FASE
    k = 4;
    switch (algorithm) {
      case 1: CountingMethods::k4(h); break;
      case 2: CountingMethods::k4ESU(h); break;
      case 4: CountingMethods::k4Fase(h); break;
      default: CountingMethods::k4Fase(h); break;
    }
  }
...
void CountingMethods::findMotifs(Hypergraph& h, int motifSize, bool detailedOutput, bool significance_profile, int randomNetworks, int randomShuffles, int algorithm, ostream& out) {
  cout << "Task: Motif discovery" << endl;
  auto startTime = steady_clock::now();
  map<int, long long> census;
  if (motifSize == 3) {
    switch (algorithm) {
      case 1: census = CountingMethods::k3(h); break;
      case 2: census = CountingMethods::k3ESU(h); break;
      case 3: census = CountingMethods::k3Triangle(h); break;
      case 4: census = CountingMethods::k3Fase(h); break;
      default: census = CountingMethods::k3Triangle(h); break;
    }
    ...
    switch (algorithm) {
      case 1: census = CountingMethods::k4(h); break;
      case 2: census = CountingMethods::k4ESU(h); break;
      case 4: census = CountingMethods::k4Fase(h); break;
      default: census = CountingMethods::k4Fase(h); break;
    }
  ...
  for (int i = 0; i < randomNetworks; i++) {
    ...
    if (motifSize == 3) {
      switch (algorithm) {
        case 1: count = CountingMethods::k3(tmp); break;
        case 2: count = CountingMethods::k3ESU(tmp); break;
        case 3: count = CountingMethods::k3Triangle(tmp); break;
        case 4: count = CountingMethods::k3Fase(tmp); break;
        default: count = CountingMethods::k3Triangle(tmp); break;
      }
    } else {
      switch (algorithm) {
        case 1: count = CountingMethods::k4(tmp); break;
        case 2: count = CountingMethods::k4ESU(tmp); break;
        case 4: count = CountingMethods::k4Fase(tmp); break;
        default: count = CountingMethods::k4Fase(tmp); break;
      }
    }
    for (auto& [a, b] : census) {
