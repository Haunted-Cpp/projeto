#pragma once

#include "Settings.hpp"

class ESU {
private:
  static std::chrono::time_point<std::chrono::steady_clock> startAlgo;
  static int f[MAX_INPUT_N];
  static std::vector<int> subgraph;
  static std::vector< std::vector<int> > g;
  static int K; // Subgraph size
  static int V; // Initial node
  
  static const int CLASSES = 180;

  static int SC;
  static int EC;
  static void enumerateSubgraphs(std::vector<int>&);
  static void setupAndRun(const std::vector< std::vector<int> >& , int);
  static Hypergraph h;
  //static std::map< int, long long> counterHyper;
  static vector<long long> counterHyper;
  static std::unordered_set< std::vector<int>, HashFunction > visited;
  static std::map<std::string, long long> FaSE(const std::vector<std::pair<int, int> > edges, int k);
  static Hypergraph binaryToHyper(std::string str, int k);
  static void clearDataStruct();
  static void computeSize(int motifSize);
  static std::vector< std::vector< std::pair<int, int> > > startEdgeGraphSubgraphs(Hypergraph&, int);
  static std::vector< std::pair<long long, std::string> > getEquivalenceClass(const std::vector< std::vector<int> >&, int);
  static std::vector< std::vector< std::pair<int, int> > > getAllSubgraphs(const std::vector< std::vector<int> >&, int);
  static std::vector<long long> k3(Hypergraph&);
  static std::vector<long long> k3Fase(Hypergraph&);
  static std::vector<long long> k3Triangle(Hypergraph& inputGraph);
  static std::vector<long long> k3ESU(Hypergraph& inputGraph);
  static std::vector<long long> bruteForce3(Hypergraph& inputGraph);
  static std::vector<long long> k4(Hypergraph&);
  static std::vector<long long> k4Fase(Hypergraph&);
  static std::vector<long long> k4ESU(Hypergraph& inputGraph);
  static std::vector<long long> bruteForce4(Hypergraph& inputGraph);
  static void k3IntermediateForm(Hypergraph& inputGraph);
  static void k4IntermediateForm(Hypergraph& inputGraph);
  static void printResults(std::chrono::time_point<std::chrono::steady_clock>, std::chrono::time_point<std::chrono::steady_clock>, std::vector<long long>, int, bool, std::ostream&);
public:
  static void networkCensus(Hypergraph&, int, bool, int, std::ostream&);
  static void findMotifs(Hypergraph&, int, bool, bool, int, int, int, std::ostream&);
};
